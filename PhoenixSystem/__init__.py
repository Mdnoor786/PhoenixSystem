...
if ENV:
    ...
    RAW_SHASA = os.environ.get("SHASA", "")
    RAW_REDLIONS = os.environ.get("REDLIONS", "")
    SHASA = list(int(x) for x in os.environ.get("SHASA", "").split())
    PHOENIX = list(int(x) for x in os.environ.get("PHOENIX", "").split())
    REDLIONS = list(int(x) for x in os.environ.get("REDLIONS", "").split())
    MONGO_DB_URL = os.environ.get("MONGO_DB_URL")
    PHOENIX_LOGS = int(os.environ.get("PHOENIX_LOGS"))
    PHOENIX_APPROVED_LOGS = int(os.environ.get("PHOENIX_APPROVED_LOGS"))
    GBAN_MSG_LOGS = int(os.environ.get("GBAN_MSG_LOGS"))
    BOT_TOKEN = os.environ.get("BOT_TOKEN")
else:
    import PhoenixSystem.config as Config

    API_ID_KEY = Config.API_ID
    API_HASH_KEY = Config.API_HASH
    STRING_SESSION = Config.STRING_SESSION
    MONGO_DB_URL = Config.MONGO_DB_URL
    with open(os.path.join(os.getcwd(), "PhoenixSystem/elevated_users.json"), "r") as f:
        data = json.load(f)
    SHASA = data["SHASA"]
    REDLIONS = data["REDLIONS"]
    PHOENIX = data["PHOENIX"]
    PHOENIX_LOGS = Config.PHOENIX_LOGS
    PHOENIX_APPROVED_LOGS = Config.PHOENIX_APPROVED_LOGS
    GBAN_MSG_LOGS = Config.GBAN_MSG_LOGS
    BOT_TOKEN = Config.BOT_TOKEN

PHOENIX.extend(SHASA)
REDLIONS.extend(PHOENIX)

session = aiohttp.ClientSession()

MONGO_CLIENT = motor_asyncio.AsyncIOMotorClient(MONGO_DB_URL)

from .client_class import ShasaClient

try:
    System = ShasaClient(StringSession(STRING_SESSION), API_ID_KEY, API_HASH_KEY)
except:
    print(traceback.format_exc())
    exit(1)

collection = MONGO_CLIENT["Shasa"]["Main"]
...
def system_cmd(
    ...
    **args
    ...
    if allow_sibyl and allow_redlion:
        args["from_users"] = REDLIONS
    elif allow_Phoenix and allow_sibyl:
        args["from_users"] = PHOENIX
    else:
        args["from_users"] = SHASA
    if force_reply:
