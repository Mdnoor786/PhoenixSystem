from PhoenixSystem import System, session, PHOENIX, REDLIONS, PHOENIX_LOGS
from PhoenixSystem.strings import proof_string, scan_request_string, reject_string
from PhoenixSystem.plugins.Mongo_DB.gbans import get_gban, get_gban_by_proofid
import PhoenixSystem.plugins.Mongo_DB.bot_settings as db
...
async def sup(event):
    await event.reply("Hey, I am Phoenix System, [Join Support](buttonurl://https://t.meLionXsupport) [Join Update](buttonurl://https://LionXUpdates)")
...
async def inline_handler(event):
    ...
    query = event.text
    split = query.split(" ", 1)
    if event.query.user_id not in PHOENIX:
        result = builder.article(
            "Phoenix System", text="You don't have access to this cmd."
        )
    ...
    if query.startswith("proof"):
        ...
        if len(split) != 5:
            ...
            u_id, redlion, source, reason, message = split
            dict_ = {
                "u_id": u_id,
                "redlion": redlion,
                "source": source,
        ...
        result = builder.article(
            "No type provided",
            text="Use\nproof <user_id> to get proof\nbuilder id:::redlion:::source:::reason:::message",
        )
    await event.answer([result])
        ...
        if user.is_self:
            ...
            if chat["alertmode"] == "ban":
                if can_ban(event):
                    ...
                    await db.change_settings(event.chat_id, True, "warn")
            await event.respond(msg)
    elif user.id in PHOENIX or user.id in REDLIONS:
        return
    else:
        u = await get_gban(user.id)
        chat = await db.get_chat(event.chat_id)
