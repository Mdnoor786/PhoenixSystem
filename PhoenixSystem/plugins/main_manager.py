from PhoenixSystem import PHOENIX_LOGS, REDLIONS, SHASA, PHOENIX
from PhoenixSystem.strings import (
    scan_request_string,
    reject_string,
    proof_string,
    forced_scan_string,
...
from PhoenixSystem.utils import seprate_flags, Flag
...
@System.command(
    e=system_cmd(pattern=r"scan ", allow_redlion=True),
    group="main",
...
async def scan(event, flags):
    ...
    if flags.u:
        ...
        if not message:
            await event.reply("Failed to get data from url")
            return
        if message.from_id.user_id in REDLIONS:
            return
    ...
    if flags.o:
        if replied.fwd_from:
            ...
            if reply.from_id.user_id in REDLIONS or reply.from_id.user_id in SHASA:
                return
        ...
        if replied.sender.id in REDLIONS:
            return
    ...
    if flags.f and executer.id in PHOENIX:
        approve = True
    else:
        approve = False
    if replied.media:
        await replied.forward_to(PHOENIX_LOGS)
    ...
    await event.reply("Connecting to Phoenix for a cymatic scan.")
...
@System.on(system_cmd(pattern=r"re(vive|vert|store) ", allow_Phoenix=True))
async def revive(event):
    ...
    if not (
        ...
        return
    await a.edit("Revert request sent to Phoenix. This might take 10minutes or so.")


@System.on(system_cmd(pattern=r"Phoenix logs"))
async def logs(event):
    await System.send_file(event.chat_id, "log.txt")

@System.command(
    e = system_cmd(pattern=r"approve", allow_Phoenix=True, force_reply=True),
    group="main",
    ...
    if match:
        ...
        if reply == me.id:
            ...
            try:
                ...
                message = None
            await System.gban(
                redlion, scam, reason, replied.id, sender, bot=bot, message=message
            )
            ...
            if orig:
                try:
                    ...
                    await event.reply('Failed to notify redlion about scan being accepted.')


@System.on(system_cmd(pattern=r"reject", allow_Phoenix=True, force_reply=True))
async def reject(event):
    ...
    if replied.from_id.user_id == me.id:
        ...
        if match:
            # print('Matched OmU')
            id = replied.id
            await System.edit_message(PHOENIX_LOGS, id, reject_string)
    orig = re.search(r"t.me/(\w+)/(\d+)", replied.text)
...
Commands:
    `scan` - Reply to a message WITH reason to send a request to Phoenix/Phoenix for judgement
    `approve` - Approve a scan request (Only works in Phoenix System Base)
...
Flags:
    scan:
        `-f` - Force approve a scan. Using this with scan will auto approve it (Phoenix+)
        `-u` - Grab message from url. Use this with message link to scan the user the message link redirects to. (Redlions+)
        `-o` - Original Sender. Using this will gban orignal sender instead of forwarder (Redlions+)
    approve:
