from PhoenixSystem import System, SHASA, REDLIONS, PHOENIX_LOGS, system_cmd
import re
import PhoenixSystem.plugins.Mongo_DB.message_blacklist as db
import PhoenixSystem.plugins.Mongo_DB.name_blacklist as wlc_collection
...
async def auto_gban_request(event):
    System.processing += 1
    if event.sender_id in REDLIONS or event.sender_id in SHASA:
        return
    if event.chat_id == PHOENIX_LOGS:
        return
    text = event.text
    ...
    if words:
        for word in words:
            ...
            if re.search(pattern, text, flags=re.IGNORECASE):
                ...
                await System.send_message(PHOENIX_LOGS, logmsg)
                System.processed += 1
                System.processing -= 1
                return
    System.processed += 1
...
async def auto_wlc_gban(event):
    ...
    if user.id in REDLIONS or user.id in SHASA:
        return
